...
for n = 0.5:0.5:5.5
  ...
  row = 2*n;
    for nn = 0:0.01:0.1
        Cap2 = nn;
        col = 100*nn + 1;
        [NPV1(row,col),SOC1avg(row,col), Lifetime(row,col),...
            P_Miss(row,col)] = System([5 Cap2 Lim]);
...
title('LIB Average SOC with Changing SC Capacity and Control')
%% Lifetime
...
title('Lifetime of LIB with Changing SC Capacity and Control')
%% NPV
